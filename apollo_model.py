"""Apollo 音乐修复模型（频带序列建模，修复有损压缩造成的高频缺失和失真）。

模型结构来自 Apollo（Kai Li 等，ICASSP 2025，https://github.com/JusperLee/Apollo），
这里使用的是 ZFTurbo Music-Source-Separation-Training（MIT 许可，PyPI 包 msst）里的实现，
只去掉了训练框架的依赖和调试输出。权重在第一次使用时从 Hugging Face 下载。
"""
